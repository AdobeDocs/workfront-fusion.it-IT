...
Adobe Workfront Fusion [!UICONTROL HTTP] > [!UICONTROL Crea un modulo di richiesta] è un modulo universale che consente di configurare una richiesta HTTP e inviarla a un server. La risposta HTTP ricevuta è quindi contenuta nel bundle di output.
...
Quando configuri il modulo [!UICONTROL HTTP] > [!UICONTROL Effettua una richiesta], Adobe Workfront Fusion visualizza i campi elencati di seguito. Un titolo in grassetto in un modulo indica un campo obbligatorio.
